...
export function MonthCanceledOrdersAmount() {
  ...
  return (
    <Card>
            ...
            {monthCanceledOrder ? (
        <>
           <h3 className="text-2xl font-bold">{monthCanceledOrder.amount.toLocaleString("pt")}</h3>
           <p>
            {monthCanceledOrder.diffFromLastMonth >= 0 ? (
              <>
                <span className="text-green-600">{monthCanceledOrder.diffFromLastMonth}%</span> m relação ao mês passado
              </>
            ) : (
              <>
                <span className="text-red-600">{monthCanceledOrder.diffFromLastMonth}%</span> em relação ao mês passado
              </>
            )}
           </p>
        </>
